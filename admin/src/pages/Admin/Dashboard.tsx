...
const Dashboard = () => {
   ...
   return (
      <>
         {!dashData && (
            <div className="w-full h-[calc(100vh-61px-48px)] sm:h-[calc(100vh-62.85px)] grid place-content-center">
               <PuffLoader size={58} />
            </div>
         )}
         {dashData && (
            <section className="h-[calc(100vh-61px-48px)] sm:h-[calc(100vh-62.85px)] overflow-y-scroll px-2.5 sm:px-5 py-4 w-full">
               <div className="flex flex-wrap gap-3 max-w-6xl">
                  <div className="w-full c450:w-auto flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 hoverable:hover:scale-105 transition-all">
               ...
               <div className="bg-white max-w-6xl">
                  ...
                  <div className="border border-t-0">
                     {dashData &&
                        dashData.latestAppointments.map((item, index) => (
                           <div className="flex items-center px-6 py-4 gap-3 hover:bg-gray-100" key={index}>
                              ...
                              {item.cancelled ? (
                                 <p className="text-red-400 min-h-10 flex items-center text-xs font-medium">
                                    Cancelled
                                 </p>
                              ) : item.isCompleted ? (
                                 <p className="text-green-500 min-h-10 flex items-center text-xs font-medium">
                                    Completed
                                 </p>
                              ) : (
                                 <div className="w-10 aspect-square rounded-full relative left-1 pt-[6px] cursor-pointer">
                                    <img
                     ...
                     {dashData.appointments === 0 && <div className="px-6 py-[27px] text-base">No bookings yet.</div>}
                  </div>
               </div>
            </section>
         )}
